    ...
    public void CheckCollider()
    {
        if (timer >= 2)
        {
            foreach (Collider col in Physics.OverlapSphere(transform.position, 2))
            {
                ...
                {
                    player.GetComponent<GrabberAgent>().AddNpc(col.gameObject);
                    timer = 0;
                }
                else if (col.gameObject.tag == "Player" && col.GetComponent<GrabberAgent>().amount < player.GetComponent<GrabberAgent>().amount)
                {
                    player.GetComponent<GrabberAgent>().KillPlayer(col.gameObject);
                    timer = 0;
                }
            }
        }
